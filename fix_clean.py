...
for filepath in ['main.js', 'frontend/main.js']:
    ...
    # Clean auto-gen injection
    clean_code = '''            if (!fetchedData) {
    ...
    content = re.sub(r'if \(!fetchedData\) \{\s*throw new Error\("No se pudo cargar el archivo data\.json desde ninguna fuente"\);\s*\}\s*appData = fetchedData;.*?(?=// Client-side instant auto-grader)', clean_code + '\n\n            ', content, flags=re.DOTALL)

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)

print("Replaced with clean code")
